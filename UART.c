/Author: Eddy Zhang  
//Email: eddy.zhang@mail.utoronto.ca
...
void initUart()
{
    bcm2835.UART.configure(UART_57);    // Using UART on pins 5 and 7
    bcm2835.UART.configure(9600, 8, PARITY_NONE, 1, NO_CTSRTS, serialRead); // 19200 baud worked well, no parity, 1 stop bit, 8 data bits
}
...
//        s+=byte;
        ...
        byte =bcm2835.UART.read();  // read from the UART buffer again (not sure if it's a valid character yet)
        // server.show(byte);
    }
    if (bcnt > 0)
    {
